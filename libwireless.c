...
void dump_80211_packet(char *pkt, size_t len) {
  ...
  if (wlan->frame_control == 0x0088) {
    ...
    if (htons(llc->type) == 0x1234) {
      size_t headers_size =
          rtp->it_len + sizeof(ieee80211_qos_header) + sizeof(llc_header);
      ...
      printf("IEEE802.11 src=%02x:%02x:%02x:%02x:%02x:%02x, "
             ...
             wlan->src[0], wlan->src[1], wlan->src[2], wlan->src[3],
             wlan->src[4], wlan->src[5], wlan->dst[0], wlan->dst[1],
             wlan->dst[2], wlan->dst[3], wlan->dst[4], wlan->dst[5],
             wlan->bssid[0], wlan->bssid[1], wlan->bssid[2], wlan->bssid[3],
             wlan->bssid[4], wlan->bssid[5]);
