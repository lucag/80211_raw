...
char buffer[128]; // min 46+14=60, max 1500+14=1524 (14 - ethernet header 2xMAC
                  // + TYPE)
...
int main(char argc, char **argv, char **env) {
  ...
  ret = sscanf(argv[2], "%02x:%02x:%02x:%02x:%02x:%02x", &rem_ethaddr[0],
               &rem_ethaddr[1], &rem_ethaddr[2], &rem_ethaddr[3],
               &rem_ethaddr[4], &rem_ethaddr[5]);
  if (ret < 6) {
  ...
  ret = sscanf(argv[3], "%02x:%02x:%02x:%02x:%02x:%02x", &bssid[0], &bssid[1],
  ...
  { /* get local mac address */
    ...
    printf("Local ethernet address: %02x:%02x:%02x:%02x:%02x:%02x, "
           ...
           loc_ethaddr[0], loc_ethaddr[1], loc_ethaddr[2], loc_ethaddr[3],
           loc_ethaddr[4], loc_ethaddr[5], rem_ethaddr[0], rem_ethaddr[1],
           rem_ethaddr[2], rem_ethaddr[3], rem_ethaddr[4], rem_ethaddr[5]);
  }
  ...
  send_packet->rtp.it_len =
      sizeof(radiotap_header) + 1 + 4;      /* only rate append, se next line */
  ...
  sprintf(buffer + sizeof(radiotap_header) + 1 + 4 +
              sizeof(ieee80211_qos_header) + sizeof(llc_header),
